...
def reset_password(id):
    ...
    conn = get_db_connection()
    row = conn.execute("SELECT email FROM teachers WHERE id = ?", (id,)).fetchone()
    if row:
        conn.execute("UPDATE teachers SET password = ? WHERE id = ?",
        ...
        # تُعرض كلمة المرور الجديدة مرة واحدة فقط ليبلّغها المدير للأستاذ
...
if __name__ == '__main__':
    ...
    host = '0.0.0.0'  # مطلوب للخدمات السحابية
    
    # في الإنتاج: DEBUG دائماً False
    debug_mode = False
